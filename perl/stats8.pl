...
#	
	...
	push (@{$localdata{"generalStats"}{"div"}}, \%alldata);
	push (@{$output{$shortname}{"div"}}, \%alldata);
    }
...
# get general stats, graphemes, groups and words
	...
	if(!defined $localdata{"generalStats"}{"surface"}){
	    $localdata{"generalStats"}{"surface"} = ();
	}
        $count++;
...
#	push (@{$localdata{"generalStats"}{"surface"}}, \%alldata);
	push (@{$localdata{"generalStats"}{"surface"}}, \%statdata);
...
sub getMetaData{
    ...
    $localdata{"language"} = ""; $localdata{"script"} = ""; $localdata{"writer"} = ""; $localdata{"object"} = "";
    ...
    $localdata{"object"} = $temp[0]->{att}->{type};
	...
	foreach my $i (@protocols) {
		...
		elsif ($temp eq "lang akk") {
		    $localdata{"language"} = "Standard Babylonian";
		}
	    }
	}
    }
